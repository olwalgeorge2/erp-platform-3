# Field Service Application Module

## 1. Purpose
1.1 Implement the application layer for the Field Service component within the Operations Field Service area of the Operations Service context.
...
3.2 Outbound: Domain ports implemented by `field-service-infrastructure` for persistence and external integrations.
3.3 Observability: Emits metrics, traces, and structured logs following platform instrumentation standards.

## 4. Related Modules
4.1 `field-service-domain/` - Core domain logic and business invariants.
4.2 `field-service-infrastructure/` - Adapter implementations for persistence and integrations.
